...
const AccessibleCard: React.FC<AccessibleCardProps> = ({ title, type, description, isActive, onClick }) => (
  <div
    ...
    className={`h-[82px] w-full flex justify-between  p-4 rounded-3xl border-2 ${isActive ? 'bg-[#f9f5ff] dark:bg-[#4c1d95]/20 border-[#EAEEFB]' : 'bg-white dark:bg-gray-800 border-[#d0d5dd] dark:border-gray-700'} flex items-center  gap-4 cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500`}
  >
    <div className="flex items-center gap-4 ">
      ...
      <div className="flex flex-col">
        ...
        <div className={`flex-nowrap  text-sm font-medium  leading-[30px]`}>
        {description}
        </div>
      </div>
    </div>
    <div className={`w-5 h-5 flex items-center justify-center rounded-full border ${isActive ? 'bg-[#2E53DA] border-none dark:bg-[#4c1d95]' : 'bg-white dark:bg-gray-800 border-[#cfd4dc] dark:border-gray-500'}`}>
      {isActive && <TickIcon />}
    </div>
  </div>
);
      ...
      <Modal className="border-gray-700 dark:bg-gray-800 border !rounded-[20px]" isOpen={isModalOpen} onClose={handleCloseModal}>
        <div className="mt-5 px-[12px] w-[480px]">
          ...
          <div className='flex flex-col gap-y-3'>
            <div className="text-[#344053] dark:text-white text-base font-bold  leading-tight">{t('agent_name')}</div>
            <div className="mb-4">
              <Input
                type="text"
                value={agentName}
          ...
          <div className="flex space-x-3">
            <Button onClick={() => setIsModalOpen(false)} variant='light' radius='full' className="w-full z">
              {t('cancel')}
            </Button>
            <Button onClick={handleNextClick} disabled={isCreating} radius='full' className="w-full">
